...
void WAVTrack::load() {
    ...
    std::cout<< "[WAVTrack::load] Loading WAV: \"" << this->title << "\" at " << this->sample_rate << "Hz/"<<this->bit_depth<<"bit (uncompressed)..."<<std::endl;
...
double WAVTrack::get_quality_score() const {
    ...
    std::cout << "[WAVTrack::get_quality_score] \"" << this->title << "\" score = "<< quality_Score <<"/100"<<std::endl;
